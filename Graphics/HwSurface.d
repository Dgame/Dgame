...
public:
	...
	*/
	void free() {
		SDL_DestroyTexture(this._sdl_tex);
	}
	...
	*/
	int[2] getSize() {
		...
		SDL_QueryTexture(this._sdl_tex, &format, &access, &w, &h);
	...
	*/
	void* lock(out int pitch, const ShortRect* area = null) {
		...
		void* pixels;
		SDL_LockTexture(this._sdl_tex, rect_ptr, &pixels, &pitch);
	...
	*/
	void unlock() {
		SDL_UnlockTexture(this._sdl_tex);
	}
	...
	*/
	void update(const void* pixels, const ShortRect* rect, int pitch = -1) {
		...
		SDL_UpdateTexture(this._sdl_tex, dst_ptr, pixels, !pitch ? this.width * 4 : pitch);
	}
	...
	*/
	void setBlendMode(BlendMode bmode) {
		SDL_SetTextureBlendMode(this._sdl_tex, bmode);
	}
	...
	*/
	BlendMode getBlendMode() {
		...
		SDL_GetTextureBlendMode(this._sdl_tex, &blendMode);
	...
	*/
	void setAlphaMod(ubyte alpha) {
		SDL_SetTextureAlphaMod(this._sdl_tex, alpha);
	}
	...
	*/
	ubyte getAlphaMod() {
		ubyte alpha;
		SDL_GetTextureAlphaMod(this._sdl_tex, &alpha);
	...
	*/
	void setColorMod(ubyte r, ubyte g, ubyte b) {
		SDL_SetTextureColorMod(this._sdl_tex, r, g, b);
	}
	...
	*/
	Color getColorMod() {
		ubyte r, g, b;
		SDL_GetTextureColorMod(this._sdl_tex, &r, &g, &b);
